	...
	<view>
		<view class="forget_password">
            ...
            <view class="forget_password_confirm">
                <input type="text" :style="{'font-size':system == 'IOS' ? '8px;' : '20px'}" v-model="confirmPwd" password="true" maxlength="15" placeholder="请确认新密码" placeholder-class="placeholder_style" @blur="checkPwdSame"/>
            </view>
		...
		methods: {
            ...
            confirm(){
                if (this.checkPhone() && this.checkPwd() && this.checkPwdSame() && this.checkCode()) {
                    ...
                    let pwdCofStr = cj.MD5(this.confirmPwd).toString();
                    let phoneStr = new Buffer(this.phoneNum).toString('base64');
                    uni.request({
