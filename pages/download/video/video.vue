		...
		<view class="downLoad">
		    <view :class="[isCancel ? ' download':'']">
                <checkbox-group @change="checkboxChange">
                    <label class="checkbox" v-for="(item ,index) in downVideo" :key="index" :data-down="item.down" :data-index="index" :data-str="item.size" :data-name="item.name" :data-url="item.path" @tap="videoMore">
		...
		methods: {
            ...
            videoMore(e){
                if (!this.isCancel) {
                    ...
                    let videoNum = e.currentTarget.dataset.name;
                        ...
                        switch ( systemName ){
                            case "ANDROID":
                                if (videoDown == 1) {
                                    ...
                                    plus.android.invoke(down, 'getTSFile','',this.name,videoNum);
            ...
            polling(i){
                ...
                var videoNum = this.downVideo[i].name;
                switch ( systemName ){
                    case "ANDROID":
            ...
            deleteItem(){
                let systemName = plus.os.name.toUpperCase();
                let videoArr = [this.name];
                videoArr = JSON.stringify(videoArr);
                let videoIndex = JSON.stringify(this.selectedArr);
                switch ( systemName ){
                    case "ANDROID":
                        let deleteFun = plus.android.newObject('com.tjyz.video.download.VideoUtils');
                        let percent = plus.android.invoke(deleteFun, 'deleteFile', videoArr, videoIndex);
                        var video = plus.android.newObject('com.tjyz.video.download.VideoUtils');
                        var videoList = plus.android.invoke(video,'downloadList',this.name);
